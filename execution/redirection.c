...
/*   Updated: 2024/10/17 18:38:14 by helarras         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_closing_fds(t_context *ctx, t_ast_type last_type,
		t_ast_type current_type)
{
	if (ctx->fd[1] != 1 && (current_type == AST_OUT_RED
			|| current_type == AST_APPEND) && (last_type == AST_OUT_RED
			|| last_type == AST_APPEND))
		close(ctx->fd[1]);
	if (ctx->fd[0] != 0 && (current_type == AST_IN_RED
			|| current_type == AST_HEREDOC || current_type == AST_INQ_HEREDOC)
		&& (last_type == AST_IN_RED || last_type == AST_HEREDOC
			|| last_type == AST_INQ_HEREDOC))
		close(ctx->fd[0]);
}
