...
bool	urslv_should_merge(t_resolver *resolver)
{
	if (!resolver->current || !resolver->next)
		return (false);
	if ((resolver->current->state == IN_DQUOTES
			|| resolver->current->state == IN_SQUOTES
			|| resolver->current->type == WORD
			|| resolver->current->type == VARIABLE
			|| resolver->current->type == STATUS)
		&& (resolver->next->state == IN_DQUOTES
			|| resolver->next->state == IN_SQUOTES
			|| resolver->next->type == WORD || resolver->next->type == VARIABLE
			|| resolver->next->type == STATUS))
		return (true);
	return (false);
}
...
bool	urslv_should_expand(t_resolver *resolver, bool hd_skip)
{
	...
	return ((current->type == VARIABLE || current->type == STATUS)
		&& current->state != IN_SQUOTES);
}
